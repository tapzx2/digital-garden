...
TARGET_DIR="~/digital-garden/content"
...
process_markdown() {
    ...
    if awk '/^---/{f=!f} f && /tags:/,/^---/' "$file" | grep -q '\- public'; then
        ...
        relative_path="${file#$SOURCE_DIR/}"  # Strip the source directory prefix
        target_file_dir="$TARGET_DIR/$(dirname "$relative_path")"  # Get the directory path
        target_file_path="$TARGET_DIR/$relative_path"  # Full target path

        mkdir -p "$target_file_dir"  # Create necessary directories
        cp "$file" "$target_file_path"  # Copy the file to the target location

        echo "Copied $file to $target_file_path" >> $LOG_FILE
...
cleanup_target_dir() {
    ...
    find "$TARGET_DIR" -type f | while read -r target_file; do
        # Strip the TARGET_DIR prefix to get the relative path
        relative_path="${target_file#$TARGET_DIR/}"

        # Determine the corresponding file path in SOURCE_DIR
        source_file="$SOURCE_DIR/$relative_path"

        # Check if the file exists in SOURCE_DIR with the exact same relative path
        if [ ! -f "$source_file" ]; then
            # If the file does not exist in SOURCE_DIR, delete it from TARGET_DIR
            rm "$target_file"
            echo "Deleted $target_file from $TARGET_DIR" >> $LOG_FILE
        else
            echo "$target_file exists in SOURCE_DIR, no action taken" >> $LOG_FILE
        fi
    done
}
