---
tags:
  - balboa
  - public
---
This is a expanded and heavily modified version of Bobby and Kate's syllabus. The progression I use for myself in practice is generally this one: [[Balboa Partnership Progression]].
## Purest Bal - most in 6 directions
- up basic
- singles: paddles, pendulum, biases
- slows: walks, sways
- down basic
- crab walk
- Come Around
- reverse come around
## Middle Ground
- 1/2 up basic 1/2 out and in (this is for transitions) (six directions)
- Out and Ins (six directions)
- Lollies “Lolly Kicks”) https://balboa.org.uk/dancer/lawrence-wise
- Fall Off The Log aka Hal Kicks
- Cross Overs
- Mess Around
- Grape Vine
## Bal Swing
- toss out  [variations](https://www.youtube.com/watch?v=7IRiInOW_9k) (slide, pop from r, release, l to r cross hand, behind the back) 
- reverse toss out (+ lunge)
- right turn (from grape vine and cross over)
- cross body lead inside turn
- left turn (slow! not quick)
- swivels (with up, down, slow, quick, and syncopated, rhythms)
- send out (very modern!! 5 versions)
## Bal Swing Turn Expansion Pack
- left turn (quick version)
- inside turn from right check position
- multi turns (both directions, no hands continue, leader/follower hand switch)
- texas tommy toss out (quick, and hal version)
- texas tommy check
- helwig

- cross hand, over head fake, scoop with all quicks
- wrap in (up, down, quick timings + in a circle, in a line + over the head first, from texas tommy first + using different hand positions)
	- i.e. toss out slide to hold hand, do second turn to come around
## Pure Bal(ish) Expansion Pack
- Vaus Ponies
- Thirds
- Drags
- MickeyKelly2000BasicBalboaRhythmsAndMusicality|triple steps (both lead and unlead)
- Step point
- leader crosses
- close position volcada (L position crab with left foot)
- side/forward/back breaks
- synchopated crabs
## Idiosyncratic Stuff
@Willie Desatoff kicks in out and ins
Pivots (right and left)
Balboa Drops
Balboa Advanced Rhythms
balboa Starter step (this is a hard maybe today since it starts with a rock step, but was perhaps popular at one time)
## Power Move Expansion Pack
Balboa Power Moves
